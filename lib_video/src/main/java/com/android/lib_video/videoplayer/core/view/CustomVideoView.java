package com.android.lib_video.videoplayer.core.view;
...
import com.android.lib_video.R;
...
public class CustomVideoView extends RelativeLayout
        ...
        TextureView.SurfaceTextureListener/* 视频专有*/{
    ...
    private Surface videoSurface; //设置到mediaplayer上 传送到surfaceview上
    ...
    private int mScreenWidth, mDestationHeight; //保证宽高比
    ...
    private boolean mIsComplete; //是否完成
    private int mCurrentCount;
    private int playerState = STATE_IDLE;

    private MediaPlayer mediaPlayer;
    private ADVideoPlayerListener listener; //
    private ScreenEventReceiver mScreenReceiver; //监听屏幕亮灭的广播接受者
...
public CustomVideoView(Context context) {
    ...
    private void initData() {
        DisplayMetrics dm = new DisplayMetrics(); //播放相关设置
        WindowManager wm = (WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE);
        wm.getDefaultDisplay().getMetrics(dm);
        mScreenWidth = dm.widthPixels; //初始化宽高
    ...
    @Override
    //第一优先级 surfaceTexture加载就绪后才能进行下一步 加载视频
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int i, int i1) {
        videoSurface = new Surface(surface);
        checkMediaPlayer(); //检查是否创建
        mediaPlayer.setSurface(videoSurface); //设置surface
        load(); //加载
    }
    ...
    private void registerBroadcastReceiver() {
        if (mScreenReceiver == null) {
            ...
            filter.addAction(Intent.ACTION_SCREEN_OFF); //监听屏幕的亮灭
    ...
    public void seekAndResume(int position) {
        if (mediaPlayer != null) {
            showPauseView(true);
            entryResumeState();
            mediaPlayer.seekTo(position); //调到对应播放的位置position
            mediaPlayer.setOnSeekCompleteListener(new MediaPlayer.OnSeekCompleteListener() {
                @Override
                public void onSeekComplete(MediaPlayer mp) {
                    mediaPlayer.start();
    ...
    public void playBack() {
        setCurrentPlayState(STATE_PAUSING);
        if (mediaPlayer != null) {
            mediaPlayer.setOnSeekCompleteListener(null); //关闭监听
            mediaPlayer.seekTo(0); //回到初始位置
            mediaPlayer.pause();
        }
        this.showPauseView(false);
    }
